...
class tickerArea(Gtk.DrawingArea):
    def __init__(self, msg, fsize):
...
class pdfWedgit(Gtk.ScrolledWindow):
    def __init__(self, file, w, h):
        ...
        print("file:///" + path.join(media_path, file))
...
class Stage(Gtk.Window):
    def __init__(self):
        ...
        webview.set_size_request(400, 500)
        webFrame.add(webview)
        fixCon.put(webFrame, 0, 10)
        ...
        fixCon.put(self.pdfwedgit_1, 500, 10)
        ...
        fixCon.put(videowidget1, 800, 10)

        tickerBox = tickerArea("香港九龍尖沙咀東麼地道", 38)
        tickerBox.set_size_request(self.stageW, 100)
        fixCon.put(tickerBox, 0, 668)

        self.show_all()
        self.run = True
        # self.fullscreen()
        # self.sched.add_job(some interval function, "interval", seconds=1)
        # self.sched.start()
