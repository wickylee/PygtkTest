...
class CSSBasicsApp:
    def __init__(self):
        ...
        css_path = os.path.join(app_path, "hello.css")

        # my_file = os.path(css_path)
        print("css_path:" + css_path)
        if os.path.isfile(css_path):
            print(css_path)

        # bytes = Gio.resources_lookup_data(css_path, 0)  # "data/css_basics.css", 0)
        # bytes = Gio.resources_lookup_data("hello.css", 0)
        buffer.set_text(
            "bytes.get_data().decode(utf-8))"
        )  # bytes.get_data().decode("utf-8"))
    ...
    def show_parsing_error(self, provider, section, error, buffer):
        start = buffer.get_iter_at_line_index(
            section.get_start_line(), section.get_start_position()
        )

        end = buffer.get_iter_at_line_index(
...
def main():
    ...
    # appWin.set_position(Gtk.WindowPosition.CENTER)
    # appWin.set_resizable(False)
    # appWin.show_all()
    # Gtk.main()
